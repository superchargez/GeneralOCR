...
num_conf = '-l eng --psm 6 -c tessedit_char_whitelist=" 0-123456789" -c tessedit_char_blacklist={abcdefghijklmnopqrstubwxyzABCDEFGHIJKLMNOPQRSTUBWXYZ}'
# for windows, if tesseract library is not in path environment then use this otherwise ignore
# path_to_exe = r'E:\Program Files\Tesseract-OCR\tesseract.exe'
...
name_conf = '-l eng --psm 6 -c tessedit_char_whitelist=" abcdefghijklmnopqrstubwxyzABCDEFGHIJKLMNOPQRSTUBWXYZ" -c tessedit_char_blacklist={0-123456789}'
                                ...
                                if charIndex < len(r[3]) and char.lower().strip() == r[3][charIndex].lower().strip():
                                    ...
                                    if char_count > len(r[3])*.5:
                                        ...
                                        if r[3] == 'Name':
                                            roi_field_text = tess.image_to_string(roi_field, config=config).strip().replace('\n', ' ')
                                            text_named_roi_field = resized[b+r[0][1]-h:b+h+r[0][1], w+a+r[0][0]:w+a+r[0][0]+w+w+w+w]
                                            text = tess.image_to_string(text_named_roi_field, config=name_conf).strip()
                                            text = text.replace(',', '').replace('.', '').replace('  ','').replace('\n', ' ')
                                            if text == '': text = 'empty'
                                            myData.append(text)
                                            # print(f'{r[3]} {text}')
                                        ...
                                        if r[3] == 'CNIC':
                                            ...
                                            # text_named_roi_field = resized[b+r[0][1]-h:b+h+r[0][1], w+a+r[0][0]:w+a+r[0][0]+w+w+w+w]
                                            text = tess.image_to_string(text_named_roi_field, config=num_conf).strip()
                                            text = text.replace(',', '').replace('.', '').replace('  ','').replace('\n', ' ')
                                            if len(text) < 13: text = text + " invalid CNIC"
                                            if text == '': text = 'empty'
                                            myData.append(text)
                                            # print(f'{r[3]} {text}')
                                        ...
                                        if r[3] == 'Email':
                                            ...
                                            text = tess.image_to_string(text_named_roi_field, config=config).strip()
                                            text = text.replace(',', '').replace('.', '').replace('  ','').replace('\n', ' ')
                                            if set("@.").issubset(text): text = text
                                            else: text=text+" invalid email"
                                            if text == '': text = 'empty'
                                            myData.append(text)
                                            # print(f'{r[3]} {text}')
                                        ...
                                        if r[3] == 'Mobile':
                                            ...
                                            text = tess.image_to_string(text_named_roi_field, config=num_conf).strip()
                                            text = text.replace(',', '').replace('.', '').replace('  ','').replace('\n', ' ')
                                            if text.startswith(("3", "0")): text = text
                                            else: text = text + " invalid mobile"
                                            if len(text) < 10: text = text + " invalid mobile"
                                            if text == '': text = 'empty'
                                            myData.append(text)
                                            # print(f'{r[3]} {text}')
